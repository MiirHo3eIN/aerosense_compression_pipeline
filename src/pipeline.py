...
def main(dataset_config, model_config): 
    ...
    train , y_train = dataset.TimeseriesSampledCpWithLabels(dataset_config.path, dataset_config.train_exp, 20, 800)
    test , y_test = dataset.TimeseriesSampledCpWithLabels(dataset_config.path, dataset_config.test_exp, 20, 800)
    ...
    print(f"Feature Space Dims: {train_features.shape}")
    # train_features, test_features = utils.rocket_feature_extraction(train_rocket = train, test_rocket = test)
